...
Metoda hybrydowa należy do grupy metod zespołowych (ensemble methods), których celem jest połączenie wyników wielu klasyfikatorów w celu poprawy jakości klasyfikacji. W przedstawionym rozwiązaniu wykorzystano trzy wcześniej zbudowane modele: wielomianową regresję logistyczną, las losowy (Random Forest) oraz sieć neuronową.

Każdy z modeli generował niezależną predykcję klasy dla obserwacji ze zbioru testowego. Następnie zastosowano mechanizm głosowania większościowego (majority voting). Ostateczną klasę przypisywano na podstawie największej liczby głosów oddanych przez poszczególne modele.

Przykładowo, jeśli dwa spośród trzech klasyfikatorów wskazywały tę samą klasę, była ona przyjmowana jako wynik końcowy. Takie podejście pozwala ograniczyć wpływ błędów pojedynczego modelu oraz zwiększyć stabilność procesu klasyfikacji.

Zastosowanie metody hybrydowej umożliwia wykorzystanie zalet różnych algorytmów. Regresja logistyczna dobrze modeluje zależności liniowe, Random Forest skutecznie identyfikuje zależności nieliniowe i interakcje pomiędzy cechami, natomiast sieci neuronowe pozwalają wykrywać bardziej złożone wzorce występujące w danych. Dzięki temu model zespołowy może osiągać lepsze wyniki niż pojedyncze klasyfikatory.
