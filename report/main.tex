...
Z uwagi na fakt, że cechy gabarytowe przyjmują wartości rzędu dziesiątek tysięcy, a bezwymiarowe współczynniki kształtu mieszczą się w wąskim przedziale $(0, 1)$, bezpośrednie zastosowanie metod opartych na odległościach (jak SVM) lub analizy kowariancji (LDA/QDA) byłoby zdominowane przez zmienne o największych barierach skali. W celu ujednolicenia wpływu wszystkich cech na proces klasyfikacji zastosowano standaryzację (centrowanie i skalowanie), przekształcając każdą zmienną tak, aby jej średnia wynosiła 0, a odchylenie standardowe było równe 1. Ponadto dla zmiennych o najwyższym współczynniku skośności uzasadnione jest rozważenie transformacji logarytmicznej przed ostatecznym skalowaniem.

\subsection{Braki danych}

Zbiór danych poddano weryfikacji pod kątem występowania brakujących wartości (rekordów typu \textit{NA} lub pustych komórek). Ze względu na to, że dane zostały wygenerowane automatycznie przez dedykowane oprogramowanie do segmentacji i ekstrakcji cech obrazu cyfrowego, zbiór jest kompletny i nie zawiera żadnych braków danych we wskazanych 12 zmiennych, co eliminuje konieczność stosowania algorytmów imputacji.

\subsection{Obserwacje odstające}

Fizyczna natura obiektów biologicznych dopuszcza występowanie anomalii rozwojowych (np. ziarna uszkodzone mechanicznie, zniekształcone lub niedorozwinięte). Identyfikacji obserwacji odstających dokonano przy użyciu reguły 1.5 rozstępu ćwiartkowego (IQR) na wykresach pudełkowych oraz wielowymiarowej analizy odległości Mahalanobisa. Obserwacje skrajnie nietypowe, będące najczęściej wynikiem błędów automatycznej segmentacji obrazu (np. złączenie dwóch nakładających się ziaren w jeden obiekt), zostały usunięte ze zbioru treningowego, aby zapobiec zaburzeniu granic decyzyjnych budowanych klasyfikatorów.
